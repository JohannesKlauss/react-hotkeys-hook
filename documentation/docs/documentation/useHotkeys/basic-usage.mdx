...
`useHotkeys` supports the following modifiers:
...
`cmd` and `command` as well as `ctrl` and `control` can be used synonymously.
...
`alt` and `option` are not identical! To ensure compatability between different operating systems, you need to understand
that i.e. the `alt` modifier won't
work with macOS. You need to use `option`. Also most shortcuts on Windows and Linux based systems that start with the
`ctrl` modifier start with `command` on macOS. This is likely to change in version 4 with the goal of unifying at least `alt` and
`option`.
:::
