...
:::hint Setting `keydown` __and__ `keyup`
...
There are tons of keyboard layouts all across different languages and operating systems. For example you reach the
hashtag sign `#` via `Shift+3` on a US keyboard, but on a German keyboard it has its own dedicated key. On the other hand
in a German keyboard layout you reach the character `[` via `Option+5`, but on windows you have your own key for that.
...
But we could also say that we want to listen to the `#` character, but only if the user presses `Shift+3`, no matter the
produced character. In this case we do need to listen to the modifier.
