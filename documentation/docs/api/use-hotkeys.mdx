...
Pass a function `filter` to determine if the callback should get triggered. Return `false` to **prevent** the execution
of the callback and `true` to **allow** the callback to be triggered.
...
:::info Setting `keydown` **and** `keyup`
If we set `keyup` to `true` and **don't** set the `keydown` prop (leaving the default), React Hotkeys Hook will assume
...
function App() {
  ...
  return (
    <div>
      ...
      <div
        style={{ padding: '30px', background: 'crimson' }}
        ref={ref}
        tabIndex={0}
      >
        Focusing this area will trigger the hotkey.
