...
We can also listen to keystrokes in a sequential way. This means that the user has to press the keys in the order we specify. To specify a sequential hotkey, we use the `>` character.
...
In the example above, the user has to press the `g` key, then the `h` key and then the `i` key in order to trigger the callback.

#### Timeout
The default timeout for the sequential hotkey is `1000ms`. If the user doesn't press the next key in time, the sequence is aborted.
